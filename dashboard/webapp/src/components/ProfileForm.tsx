...
const EMPTY: Profile = {
  ...
  excluded_trim_keywords: [], excluded_years: [], show_financing: true,
  down_payment: null, email_only_on_new_or_drops: false,
}
...
export function ProfileForm({ initial, docs, onSave, onClose }: Props) {
  ...
  const setVehicle = (i: number, idx: 0 | 1, val: string) => {
    const updated = form.vehicles.map((v, j) =>
      j === i ? (idx === 0 ? [val, v[1]] : [v[0], val]) as [string, string] : v
    )
  ...
  const removeVehicle = (i: number) => {
    ...
    set('vehicles', updated)
    set('model_preference', form.model_preference.filter(m => updated.map(v => v[1]).includes(m)))
  }
    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="relative ml-auto w-full max-w-xl bg-white h-full overflow-y-auto shadow-xl z-50 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          {/* Email alert mode */}
          <Field label="Email alert mode" hint="Only email when new listings appear or an existing vehicle drops in price.">
            <label className="flex items-center gap-2 text-sm cursor-pointer mt-1">
          ...
          {/* Reference doc */}
          <Field label="Reference doc" hint='Auto-discover finds matching files by vehicle make/model. Select manually to override.'>
